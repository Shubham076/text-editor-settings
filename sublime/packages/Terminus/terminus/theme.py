...
class TerminusGenerateThemeCommand(sublime_plugin.WindowCommand):
    def run(self, theme=None, remove=False, force=False):
        ...
        if theme == "user":
            ...
            selection = "#665C54" if is_dark_theme else "#DBDBDB"
            white = selection
            light_black = "#888888"
            ...
            if window:
                ...
                selection = view.style().get("selection", selection)
                highlight = view.style().get("line_highlight", selection)
                light_black = view.style_for_scope("comment")["foreground"]
                white = highlight

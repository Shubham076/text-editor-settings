...
TERMINUS_STATUS_KEY = "terminus_panel"


def find_terminus_panel_name(window):
    """Return the name of the currently open Terminus output panel, if any."""
    if not window:
        return None
    ...
    panel_view = window.find_output_panel(name)
    if panel_view and panel_view.settings().get("terminus_view"):
        return name
    return None


def set_terminus_status(view, panel_name):
    """Pin ``Terminal: <panel_name>`` to the status bar.

    Problem:
        ``Window.status_message()`` only writes to the transient, window-global
        message slot: it expires on its own and is clobbered by any other plugin
        writing a message, so the label just flashed for a moment and vanished.
        Switching to ``View.set_status()`` on the terminal view fixed the
        lifetime but showed nothing at all, because status keys are stored per
        view and Sublime only renders the keys of ``Window.active_view()`` --
        and ``active_view()`` deliberately skips output panels, returning the
        regular file view behind the panel even while the panel has focus. The
        status was therefore stored on a view Sublime never reads it from.

    Solution:
        Write the status key to the window's active view as well as to the
        terminal view itself, so it lands on the view that actually gets
        rendered. ``TerminusStatusBarListener.on_activated`` then re-applies it
        as focus moves between views, since each view carries its own keys.
    """
    status = "Terminal: {}".format(panel_name)
    view.set_status(TERMINUS_STATUS_KEY, status)
    window = view.window() or sublime.active_window()
    if not window:
        return
    active_view = window.active_view()
    if active_view and active_view.id() != view.id():
        active_view.set_status(TERMINUS_STATUS_KEY, status)


def clear_terminus_status(window):
    """Erase the pinned status from every view and panel of ``window``.

    The key is stamped onto whichever view happens to be active at the time, so
    once the terminal is gone it has to be erased everywhere. Otherwise a stale
    ``Terminal: ...`` label sticks to every view that was focused while the
    terminal was alive.
    """
    if not window:
        return
    for view in window.views():
        view.erase_status(TERMINUS_STATUS_KEY)
    for panel in window.panels():
        if not panel.startswith("output."):
            continue
        panel_view = window.find_output_panel(panel[len("output."):])
        if panel_view:
            panel_view.erase_status(TERMINUS_STATUS_KEY)


def refresh_terminus_status(window):
    """Recompute the pinned label, e.g. after a terminal is closed.

    ``terminus_close`` destroys the output panel directly instead of closing a
    view, so no close event fires and the label of the destroyed terminal would
    otherwise stay pinned to every view it was stamped on.
    """
    if not window:
        return
    clear_terminus_status(window)
    panel_name = find_terminus_panel_name(window)
    if not panel_name:
        return
    panel_view = window.find_output_panel(panel_name)
    if panel_view:
        set_terminus_status(panel_view, panel_name)
...
class TerminusRenderCommand(sublime_plugin.TextCommand, TerminusViewMixin):
    ...
    def run(self, edit):
        ...
        # Show panel identification as a persistent status bar entry
        if panel_name:
            set_terminus_status(view, panel_name)
...
class TerminusStatusBarListener(sublime_plugin.EventListener):
    """Keep the Terminus panel name pinned to the status bar."""

    def on_activated(self, view):
        """Re-apply the status to whichever view just gained focus."""
        if not view:
            return
        ...
        panel_name = find_terminus_panel_name(window)
        if panel_name:
            # `view` may itself be the output panel, whose status keys Sublime
            # never renders, so go through the helper to also stamp the active
            # view.
            set_terminus_status(view, panel_name)
        else:
            clear_terminus_status(window)
    ...
    def on_pre_close(self, view):
        """Drop the label when a terminal view or panel is closed."""
